...
**consume** `@flowds/*` como dependencia — nunca redefine un token ni una prop; los importa.

- Repo del sistema (publica `@flowds/*`): https://github.com/alohasoyrico-eng/Design-system-multiplataforma-desde-cero
- Contrato: `@flowds/design-system` (y capas sueltas) en npm, versionado con SemVer (lockstep).

## Estado

Las dependencias `@flowds/*` están declaradas a `^0.1.0` pero **aún no publicadas** en npm.
...
desde `@flowds/tokens/json`), no se escribe a mano. Lo escrito a mano se limita al juicio humano:
